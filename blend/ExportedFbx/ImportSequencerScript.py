...
#Use this command : py "C:\Users\chime\Documents\Unreal Projects\snrydemo\blend\ExportedFbx\ImportSequencerScript.py"
...
def CreateSequencer():
		...
		for camera in ImportedCamera:
			if camera[0] == 'Camera':
				...
				camera_binding_id.set_editor_property('guid', camera[1].get_id())
	...
	#Select and open seq in content browser
		if int(unreal.SystemLibrary.get_engine_version()[:4][2:]) >= 26:
			unreal.AssetEditorSubsystem.open_editor_for_assets(unreal.AssetEditorSubsystem(), [unreal.load_asset(seqPath+'/'+seqName.replace('.',''))])
		else:
